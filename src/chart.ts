...
const selectedCountEl =
	document.querySelector<HTMLDivElement>("#selectedCount");
...
async function loadData() {
	try {
		...
		packages.forEach(pkg => {
			...
			packageData[name] = {
				weekly: {},
				enabled: preEnabled.includes(name),
			};
		});
		allPackages.sort();
		...
		document
			...
			.addEventListener("input", e => {
				...
				document.querySelectorAll(".package-item").forEach(item => {
					...
					(item as HTMLElement).style.display = text.includes(term)
						? "flex"
						: "none";
				});
			});
		...
		const processSinglePackage = async (
			pkg: string,
			...
			try {
				...
				if (Object.keys(weekly).length > 0) {
					...
					if (
						newWeeks.length > allWeeks.length ||
						newWeeks.some((w, i) => w !== allWeeks[i])
					) {
						allWeeks.length = 0;
		...
		const priorityPackages = packages.filter(p =>
			preEnabled.includes(removePrefix(p)),
		);
		const otherPackages = packages.filter(
			p => !preEnabled.includes(removePrefix(p)),
		);
...
function createChart() {
	const ctx = document
		.querySelector<HTMLCanvasElement>("#downloadsChart")
		.getContext("2d")!;
